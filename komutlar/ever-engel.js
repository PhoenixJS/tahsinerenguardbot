...
exports.run = async (client, message, args) => {
  ...
  if (!args[0]) return message.channel.send('Hatalı kullanım! :x: **Örnek Kullanım:** everyone-engel aç/kapat')
  
  if (args[0] == 'aç') {
    db.set(`hereengel_${message.guild.id}`, 'acik')
      message.channel.send('Everyone Engel başarıyla açıldı! :white_check_mark:')
   
  }
  if (args[0] == 'kapat') {
    db.delete(`hereengel_${message.guild.id}`, 'kapali')
      message.channel.send('Everyone Engel başarıyla kapatıldı! :x:')
