...
exports.run = (client, message, args) => {
  if(!message.member.permissions.has("ADMINISTRATOR")) return message.reply(":x: bu komutu kullanmak için `Yönetici` yetkisine sahip olmalısın :x:")
  if (db.has(`antiraidK_${message.guild.id}`) === false) {
    return message.channel.send(
      "Anti-raid açılmamış. Açmak için **.anti-raid aç**"
    );
  }
  if (!args[1]) return message.reply("Hey Sen Lütfen Bi Bot İd'si Gir :warning:");
